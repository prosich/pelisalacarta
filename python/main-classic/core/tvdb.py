...
def set_infoLabels_item(item):
    ...
    if 'infoLabels' in item and 'season' in item.infoLabels:
        ...
        if item.infoLabels['episode']:
            ...
            if data_season and 'image_season_%s' % int_season in data_season:
                item.thumbnail = HOST_IMAGE + data_season['image_season_' % int_season][0]['fileName']
    ...
    # Buscar...
        ...
        # Busquedas por ID...
        if (not otvdb_global or otvdb_global.get_id() != item.infoLabels['tvdb_id']) \
            and item.infoLabels['tvdb_id']:
            otvdb_global = Tvdb(tvdb_id=item.infoLabels['tvdb_id'])
...
class Tvdb:
    ...
    def get_list_results(self):
        ...
        if len(self.list_results) == 1 and "seriesName" in self.result:
            ...
            for i, e in enumerate(self.list_results):
                t = threading.Thread(target=sub_thread, args=(e["id"], i))
                t.start()
                l_hilo.append(t)
